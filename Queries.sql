...
/* Insight: El precio promedio equivale a 5.65. */
...
/* Insight: El precio promedio por país pone en primer lugar a Chile con un precio de 6.7 
...
/* Insight: El país con mayores ingresos ha sido USA, con un total de 523, seguido por Canadá, con 303. */
...
/* Insight: Encontramos resultados muy pares entre los distintos años expuestos. Además, aunque la cantidad vendida
...
WITH clientes AS( 
SELECT c.customerId AS 'ID', CONCAT(c.LastName, ', ', c.FirstName) AS 'Cliente', c.city AS 'Ciudad',
	COUNT(i.invoiceId) AS 'Compras', SUM(i.total) AS 'Total gastado',
...
SELECT * FROM clientes
	WHERE `Total gastado` > (SELECT AVG(`Total gastado`) FROM clientes);

/* Insight: En esta consulta encontramos que los clientes con un gasto superior al promedio total han comprado
...
SELECT g.genreId AS 'ID', g.Name AS 'Género musical', COUNT(il.invoiceLineId) AS 'Ventas',
	SUM(il.UnitPrice * il.Quantity) AS 'Total ingresado',
...
/* Insight: El género de Rock destaca claramente en ventas en comparación con el resto de géneros. De hecho, supera 
...
SELECT ar.ArtistId AS 'ID', ar.Name AS 'Artista', COUNT(il.invoiceLineId) AS 'Ventas',
	SUM(il.UnitPrice * il.Quantity) AS 'Total ingresado',
    RANK() OVER(ORDER BY COUNT(il.invoiceLineId) DESC) AS 'Ranking ventas'
...
WITH top_artista AS(
SELECT i.billingcountry AS 'País', ar.ArtistId AS 'ID', ar.Name AS 'Artista', COUNT(il.invoiceLineId) AS 'Ventas',
	SUM(il.UnitPrice * il.Quantity) AS 'Total ingresado',
...
/* Insight: Podemos observar como entre los 10 artistas más vendidos hay una fuerte presencia de precios unitarios
...
/* CONCLUSIÓN GENERAL: El análisis muestra que USA es el principal mercado en ingresos, Rock domina claramente las ventas 
por género y algunos artistas/géneros con menor volumen de ventas generan más ingresos debido a un precio unitario superior. */